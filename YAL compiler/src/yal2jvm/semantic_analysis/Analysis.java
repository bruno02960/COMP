...
public abstract class Analysis
{
	...
	/**
	 * Abstract function that will be used by the analysis classes that extend this
	 * class
	 */
	protected abstract void parse();

	/**
	 * Checks, separately, if the inheritedSymbols and the mySymbols hashMaps are
	 * set to null. If they are both true, the elements of each checked hashMap are
	 * stored inside another hashMap variable and the new variable is returned. If
	 * one check is true and the other is not, the elements of the hashMap that
	 * passed the validation will be stored inside the new hashMap variable. If they
	 * are both false, the new hashMap variable will return an empty hashMap.
	 *
	 * @return the hashmap containing all of the elements of inheritedSymbols plus
	 *         all of the elements of mySymbols. Returns an empty hashMap when both
	 *         are set to null.
	 */
	...
	/**
	 * Checks if the value of the parameter symbolId is a key of the hashMap
	 * mySymbols. If true, the value mapped to that key is stored in a new Symbol
	 * variable and is returned. If false, checks if the hashMap inheritedSymbols
	 * has a key with the parameter's value. If true, the value mapped to that key
	 * is stored in the new Symbol variable. If all checks fail, the new variable is
	 * returned with a null value.
	 *
	 * @param symbolId
	 *            id of the symbol that will be used to check if it is contained in
	 *            any of the two hashMaps(mySymbols and inheritedSymbols)
	 * @return value mapped to the key symbolId in the mySymbols hashMap or the
	 *         inheritedSymbols hashmap if the first is set to null or that key
	 *         value doesn't exit. Return value is null when the key doesn't exist
	 *         in any of the two hashMaps
	 */
	...
	/**
	 * Checks the type of the lhsTree first child. If the child node is an array
	 * access the function parseArrayAccess is returned. If the child node is a
	 * scalar access the function parseScalarAccess is returned. If the child node
	 * isn't any of the two, no function is called a null value is returned.
	 *
	 * @param lhsTree
	 *            contains the entire lhs tree
	...
	/**
	 * Checks the type of the rhsTree first child. If the child node is an
	 * "ARRAYSIZE" this node will be stored in a ASTARRAYSIZE variable. It will then
	 * be processed by the parseArraySize function and it's return value stored in a
	 * VarSymbol variable(retval). If the value of the variable retval is null, this
	 * function will return null. If not, it will return a copy of itself with the
	 * type "ARRAYSIZE".
	 * <p>
	 * If the child node isn't of type "ARRAYSIZE", all of the rhs tree children
	 * will be processed. This process will check if the children have the same type
	 * "term". Returning null when they are of different type and when they are of
	 * type array. Returning the return value of the parseTerm function of the last
	 * child processed.
	 *
	 * @param rhsTree
	 *            contains the entire rhs tree.
	 * @return null if the function fails the validations. ASTARRAYSIZE variable if
	 *         the first child is of type "ARRAYSIZE" and VarSymbol variable if not.
	 */
		...
		for (int i = 0; i < numChildren; i++)
		{
			...
			else if (!previousType.equals(symbolType))
			{
				System.out.println("Line " + child.getBeginLine() + ": Variables dont match! Variable "
						+ previousSymbol.getId() + " has type " + previousSymbol.getType() + " and " + symbol.getId()
						+ " has type " + symbol.getType() + ".");
	...
	/**
	 * Receives a variable of type "ASTARRAYSIZE" and if it is an integer, the
	 * function returns a new object of type ImmediateSymbol with that variable as a
	 * parameter for the constructor.
	 * <p>
	 * If the variable isn't an integer, then it is a scalar access. The first child
	 * of the variable will be the parameter of the return function
	 * parseScalarAccess
	 *
	 * @param arraySizeTree
	 *            contains the entire arraySize tree.
	...
	/**
	 * Receives a parameter of type "ASTTERM" and checks if it is an integer. If it
	 * is, the functions returns a new object of type immediateSymbol with that
	 * value as a parameter for the constructor. If it isn't, the type of it's first
	 * child will be checked. If it is of type "CALL", the function parseCall will
	 * process it and check for errors. If it is of type "ARRAYACCESS" the function
	 * parseArrayAccess will be returned with that child. If it is of type
	 * "SCALARACCESS" the function parseArrayAccess will be returned with that
	 * child.
	 *
	 * @param termTree
	 *            contains the entire term tree.
	...
	private VarSymbol parseTerm(ASTTERM termTree)
	{
		...
		case "CALL":
			...
			if (callRet == null)
			{
				System.out.println("Line " + astCall.getBeginLine() + ": Method " + astCall.method
						+ " is void. Expected return value.");
				ModuleAnalysis.hasErrors = true;
			}
			return callRet;
		...
		} else
		{
			...
			if (functionArguments.size() != argumentsTypes.size())
			{
				System.out.println("Line " + astarguments.getBeginLine() + ": Method " + method + " arguments number("
						+ argumentsTypes.size() + ") does not match expected number(" + functionArguments.size()
						+ ") of arguments");
				ModuleAnalysis.hasErrors = true;
				return null;
			}
			...
			for (int i = 0; i < functionArguments.size(); i++)
			{
				...
				if (!argumentType.equals(expectedArgumentType))
				{
					System.out.println("Line " + astarguments.getBeginLine() + ": Type " + argumentType
							+ " of argument " + i + 1 + " of method " + method + " call does not match expected type "
	...
	private Symbol checkSymbolExistsAndIsInitialized(SimpleNode ast, String symbolId)
	{
		...
		{
			System.out.println(
					"Line " + ast.getBeginLine() + ": Variable " + symbolId + " might not have been declared.");
			ModuleAnalysis.hasErrors = true;
			return null;
		}
		...
		{
			System.out.println(
					"Line " + ast.getBeginLine() + ": Variable " + symbolId + " might not have been initialized.");
			ModuleAnalysis.hasErrors = true;
			return null;
		}
	...
	private VarSymbol parseScalarAccess(ASTSCALARACCESS scalarAccessTree)
	{
		...
		if (sizeAccess)
		{
			...
			if (varSymbol == null)
			{
				System.out.println("Line " + scalarAccessTree.getBeginLine() + ": Variable " + id
	 ...
	 * @return
	 */
	private VarSymbol parseDeclarationAstScalarElement(ASTDECLARATION declarationTree,
			ASTSCALARELEMENT astscalarelement)
	{
	 ...
	 * @return
	 */
	private VarSymbol createSymbolForDeclarationAstScalarElement(ASTDECLARATION declarationTree,
			ASTSCALARELEMENT astscalarelement)
	{
	 ...
	 * @return
	 */
	private VarSymbol createSymbolForDeclarationAstArrayElement(ASTDECLARATION declarationTree,
			ASTARRAYELEMENT astarrayelement)
	{
		...
		{
			// if from type a[] = CONST; and variable array has no size set (its not
			// declared even)
			if (declarationTree.integer != null)
			{
				System.out.println("Line " + declarationTree.getBeginLine() + ": Variable " + astarrayelement.id
						+ " has the size not defined." + " Error assigning " + declarationTree.integer
						+ " to all elements of " + astarrayelement.id + ".");
				ModuleAnalysis.hasErrors = true;
				return null;
			}
	...
	private VarSymbol parseDeclarationSymbol(ASTDECLARATION declarationTree, VarSymbol symbol)
	{
		...
		{
			System.out.println(
					"Line " + declarationTree.getBeginLine() + ": Variable " + symbol.getId() + " already declared.");
			ModuleAnalysis.hasErrors = true;
			return null;
		}
		...
		if (symbol.getType().equals(Type.INTEGER.toString()) && symbol.isInitialized())
		{
			System.out.println("Line " + declarationTree.getBeginLine() + ": Variable " + symbol.getId()
					+ " was already initialized." + " Error assigning " + declarationTree.integer + " to the variable "
					+ symbol.getId() + ".");
			ModuleAnalysis.hasErrors = true;
			return null;
		}
		...
		if (symbol.getType().equals(Type.ARRAY.toString()) && symbol.isInitialized() == false)
		{
			System.out.println("Line " + declarationTree.getBeginLine() + ": Variable " + symbol.getId()
					+ " has the size not defined." + " Error assigning " + declarationTree.integer
					+ " to all elements of " + symbol.getId() + ".");
			ModuleAnalysis.hasErrors = true;
			return null;
		}
	...
	private boolean parseAssign(ASTASSIGN assignTree)
	{
		...
		if (lhsSymbolType.equals(rhsSymbolType) && !(rhsSymbol instanceof ImmediateSymbol)) // if both lhs and rhs have
																							// same type
		{
		...
		{
			System.out.println(
					"Line " + lhsTree.getBeginLine() + ": Variable " + lhsSymbol.getId() + " has the size not defined."
							+ " Error assigning right hand side to all elements of " + lhsSymbol.getId() + ".");
			ModuleAnalysis.hasErrors = true;
			return false;
		}
		...
		if (!(lhsSymbolType.equals(SymbolType.ARRAY.toString()) && rhsSymbolType.equals(SymbolType.INTEGER.toString()))) // for
																															// A=5;
																															// in
																															// which
																															// A
																															...
																															// to
																															// 5
			if (!rhsSymbolType.equals(SymbolType.UNDEFINED.toString())) // for A=m.f(); in which m.f() function is from
																		// another module that we not know the return
																		// value, so it can be INTEGER or ARRAY
	...
	private VarSymbol getLhsVariable(SimpleNode lhsTree)
	{
		...
		case "ARRAYACCESS":
			...
			if (symbol == null)
			{
				System.out.println("Line " + astArrayAccess.getBeginLine() + ": Variable " + id
						+ " might not have been declared.");
				return null;
			}
	...
	protected boolean parseExprTest(ASTEXPRTEST astExprtest)
	{
		...
		// if operands being tested are both array type and the comparison operator is
		// not == or !=, semantic error.
		if (lhsSymbol.getType().equals(SymbolType.ARRAY.toString()))
		{
			if (astExprtest.operation != "==" && astExprtest.operation != "!=")
			{
				System.out.println(
						"Line " + astLhs.getBeginLine() + ": Variables must be INTEGER to be compared. Variable "
								+ lhsSymbol.getId() + " has type " + lhsSymbol.getType() + " and variable "
								+ rhsSymbol.getId() + " has type " + rhsSymbol.getType() + ".");
				ModuleAnalysis.hasErrors = true;
				return false;
			}
	...
	protected void parseStmtLst(ASTSTATEMENTS astStatements)
	{
		...
		for (int i = 0; i < statementsNumChilds; i++)
		{
			...
			{
			case "WHILE":
				WhileAnalysis whileAnalysis = new WhileAnalysis(node, getUnifiedSymbolTable(),
